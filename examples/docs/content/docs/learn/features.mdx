...
Everything in the Learn section so far is available in every Vidact project with no configuration. A second group of capabilities is opt-in. They are all part of React's API and they all work the way you expect; they are just switched off until you ask for them.
...
Two reasons. First, size: a Suspense implementation, a transition scheduler, and an action queue each add code to the runtime. If your app never uses them, they should not be in the bundle, and with feature flags they are not. Second, clarity: when a teammate uses `useTransition` in a project that has not enabled `concurrent`, they get a compile error naming the flag, rather than a runtime surprise three months later.
