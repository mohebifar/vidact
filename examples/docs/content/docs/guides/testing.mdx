...
Vidact's promise is that updates are surgical. `@vidact/test-support` includes helpers to prove it in tests. `captureMutations` runs an action under a `MutationObserver` and returns the records; `assertMutationEnvelope` checks that only the expected kinds of mutation touched the expected nodes.
