...
Compiler diagnostics include a source location, an error code, and a message. This page explains common errors and how to resolve them.
...
A package you import uses code outside the supported subset. Update or replace the affected component, or report the unsupported form with a minimal reproduction. Excluding a React component package from compilation does not make it work in Vidact. `includeDependencies` is only for compatible packages that were not detected automatically; see [@vidact/vite](/docs/reference/vite).
