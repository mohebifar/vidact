...
A source-published package is eligible for Vidact compilation when its reachable code stays inside this subset. The Vite plugin qualifies such packages automatically when their `package.json` declares React. Qualification is not a package-wide support claim: compilation, SSR, browser behavior, DOM ownership, and typing require separate evidence. Packages that are precompiled against React's runtime, or that depend on rejected APIs, fail at build time.
