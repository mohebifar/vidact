...
description: The documented React-shaped subset and how each surface behaves under Vidact.
group: Reference
---

Vidact compiles a subset of React 19. This page summarizes what is in that subset, what needs a feature flag, what behaves differently, and what is rejected. Compatibility remains source-shape and target specific; a package or component is certified only to the level exercised by its repository tests.

## How to read the tables

| Status | Meaning |
| --- | --- |
| **Supported** | Works in compiled source subject to the documented notes |
...
| `forwardRef` | Adapted | Inline component functions only, as a migration aid |
| `memo` | Adapted | Accepted; no custom comparator; not a performance tool |
| `createElement` | Adapted | Statically known elements compile; a runtime element type renders through the runtime |
...
A package is eligible for Vidact compilation when its reachable code stays inside this subset. The Vite plugin detects React dependencies from package metadata and recognizes both JSX and published `react/jsx-runtime` calls. Packages that depend on rejected APIs or React internals fail at build time.

Successful compilation does not certify an entire package. The browser corpus exercises selected `@base-ui/react` button, switch, and accordion interactions. Other components and combinations need their own tests, including hydration and keyboard behavior. Run the paths your application uses against the package versions you intend to ship.
