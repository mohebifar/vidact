...
| `renderDocumentShell(context)` | built-in streaming template | Produce `beforeApplication` and `afterApplication` strings around the application stream |
...
`renderDocument` is the buffered compatibility path because it receives the
complete application string. To preserve streaming, use
`renderDocumentShell(context)` and return:

```ts
{
  beforeApplication: `<!doctype html><main id="${context.rootId}">`,
  afterApplication: `</main><script id="${context.snapshotId}" type="application/json">${context.snapshot}</script>`,
}
```

The built-in shell is sent after loaders settle and before pending application
resources settle. The marker-complete application is streamed next, followed by
the snapshot and client entry. A render failure after the first shell chunk
errors the body stream because the `200` status is already committed. `HEAD` and
navigation snapshot requests do not construct the application stream.
