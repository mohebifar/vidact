...
After hydration, a `<Link>` click requests a snapshot of the target URL's loader data. The client updates retained route components and mounts any new routes with that data. The same loaders serve both the first request and later navigations.
