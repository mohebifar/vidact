...
Vidact Start maps files in `src/routes` to URLs. File names define static paths, parameters, layouts, and catch-all routes.
...
The root layout renders inside the document's root element. The `<html>`, `<head>`, and `<body>` come from Start's streaming shell or your `renderDocumentShell` customization. The buffered `renderDocument` callback remains available when a template needs the complete application HTML; see [@vidact/start](/docs/reference/start).
...
- Shared routes and layouts remain mounted during navigation. State in a route that leaves the matched tree is disposed; see [State across navigations](/docs/start/navigation#state-across-navigations).
