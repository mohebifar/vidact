...
This page follows a component from parsed TSX through dependency analysis to generated JavaScript.
...
The compiled module imports helpers from `@vidact/runtime` for element creation, text bindings, event batching, state slots, ownership, and scheduling. It creates DOM directly. The runtime keeps owner records and declared dependencies rather than a virtual element tree.
...
Build-time analysis lets Vidact emit bindings tied to known source values. The runtime still handles scheduling, ownership, and DOM publication, but it does not need to re-run the component to discover its next output. This requires code the compiler can analyze, which limits the supported React subset.
