...
The compiled module imports a handful of small helpers from `@vidact/runtime`: element creation, text binding, event wrapping, state slots, and the scheduler. There is no reconciler, no element tree, no component instance objects, and no dependency-tracking proxy. A compiled counter, runtime included, is a few kilobytes gzipped.
