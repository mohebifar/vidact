...
Owners track the DOM, state, and cleanup work associated with a component or dynamic region. An update either preserves that owner or disposes it, depending on the region's identity.
...
These rules depend on Vidact's compiled ownership boundaries. In particular, two branches can own separate component instances even when both use the same component type.
...
Disposal runs registered cleanup work and removes owned DOM. Do not depend on child DOM remaining attached during a parent's cleanup. Writing state that belongs to a disposed owner is an error ("cannot update state after disposal" in development, `V012` in production). Cancel timers and subscriptions, and ignore late asynchronous results after cleanup.

Vidact stages construction and supported DOM writes before publication. If computation fails, staged owners are disposed; supported publication operations have rollback behavior. This does not undo arbitrary side effects in your callbacks or restore a state value already written. Use error boundaries and test the failure paths your application relies on.

## Roots

A root is the owner with no parent. It records the error callbacks and `useId` prefix for everything below it. `createRoot(...).replace(nextApp)` constructs the next application's owner before disposing the previous one. Hot module replacement uses root replacement. Vidact Start navigation retains shared route owners and replaces only the changed route suffix.
